...
const priorityByType: Record<AgentProblemType, number> = {
  high_potential: 90,
  weak_conversion: 80,
  low_exposure: 70,
  new_product_pool: 60,
...
export function buildAgentTaskPool(context: PublicTrafficDataReportContext): AgentTaskItem[] {
  ...
  for (const item of getNewProductPool(context)) {
    tasks.push({
      ...
      taskType: 'new_product_pool',
      priority: priorityByType.new_product_pool,
      reason: item.productName || '新品池待维护',
      suggestedAction: item.maintenanceStatus,
      status: '待处理',
    });
  }
