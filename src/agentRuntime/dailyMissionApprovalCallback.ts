...
export async function resolveDailyMissionApproval(
  ...
  if (decision.runId !== tag.runId || decision.runId !== run.runId) {
    throw new Error(`Decision ${tag.decisionId} does not belong to Daily Mission run ${tag.runId}.`);
  }
  ...
  const runResults = results.filter((entry) => entry.runId === run.runId);
  const pending = runResults.some((entry) => executionStatus(entry) === 'pending_confirmation' || executionStatus(entry) === 'processing');
  const terminalDecisionIds = new Set(results
    .filter((entry) => entry.runId === run.runId && (executionStatus(entry) === 'executed' || executionStatus(entry) === 'failed'))
  ...
  if (!pending && allApprovedTerminal) {
    const failedDecisionIds = new Set(runResults.filter((entry) => executionStatus(entry) === 'failed').map((entry) => entry.decisionId));
