        ...
        messages: [
          {
            ...
            content: [
              ...
              'For data/strategy tool follow-up steps, prefer stable metadata fields: use publicTraffic.windowAggregate productIds, product.rankBestSameSku bestProductId/sameSkuGroupId/productIds, and strategy.refreshCandidateExplain candidateProductIds or missing30dDashboardProductIds instead of guessing nested item shapes.',
              ...
              'For activity refresh goals, use operations.refreshActivityPlan with explicit metric, operator, value, and windowDays. 访问量/公域访问量为0 → metric=publicVisits, operator=eq, value=0. 曝光量为0 → metric=exposure. 创单/创建订单数为0 → metric=createdOrders. 订单金额/金额为0 → metric=amount unless the user explicitly names 创建金额/签约金额/审核金额/发货金额, which map to createdOrderAmount/signedOrderAmount/reviewedOrderAmount/shippedOrderAmount. 不得将用户指定指标改写为创单、金额或其它指标; if the requested metric is unclear or unsupported for automatic delist, ask for clarification instead of substituting. When the user names a product/model/group, pass query or sameSkuGroupId to keep the plan targeted. The plan returns a strategy choice card only for executable delist metrics; confirmed execution still uses operations.refreshActivityExecute and may skip blocker groups for partial execution.',
