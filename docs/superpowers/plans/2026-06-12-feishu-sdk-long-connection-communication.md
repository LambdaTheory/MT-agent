...
- Modify: `package.json`
  ...
  - Add root override forcing `axios` to `1.17.0`.
- Modify: `package-lock.json`
  - Generated by `npm install @larksuiteoapi/node-sdk@1.66.1` and `npm install` after adding the override.
- Modify: `.env.example`
...
```powershell
npm install @larksuiteoapi/node-sdk@1.66.1
```

Expected: `package.json` gains `@larksuiteoapi/node-sdk` under `dependencies`, and `package-lock.json` is updated.

- [ ] **Step 2: Override vulnerable axios transitively used by the SDK**

Add this root `package.json` override and regenerate the lockfile:
...
Expected: `package-lock.json` resolves `node_modules/axios` to `1.17.0`.

- [ ] **Step 3: Document SDK mode env**
...
- [ ] **Step 4: Verify install and metadata compile**
