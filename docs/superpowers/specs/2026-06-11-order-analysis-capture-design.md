...
1. 订单分析四页指标抓取，落盘 JSON，并追加到日报 xlsx 的单个 `订单分析` sheet。
2. 日报 xlsx `商品明细` sheet 表头汉化。
3. 访问页 4 个新金额列接入商品级数据管线（仅抓取、落盘、展示，不改诊断规则）。

明确不做：

- 订单分析数据不进飞书卡片、不进 Markdown、不进今日漏斗（攒几天真实数据后另行设计接入口径）。
