...
MT-agent 是一个面向运营场景的 `Node.js + TypeScript` 自动化项目，当前主要服务于美团/支付宝侧商品与公域流量分析，并通过飞书完成日报投递、只读查询、任务提醒，以及部分活动页自动化辅助。
...
- 已完成：分析、报表、通知、只读查询、差异化定价活动自动化辅助、部分带确认的工作流。
...
- 高风险变更型能力虽然有基础工作流，但不建议在未审批/未回归前直接自动执行。
...
这是一个已经具备“抓取数据 -> 生成运营日报 -> 飞书分发 -> Bot 只读查询 -> 部分自动化辅助”闭环的运营 Agent 项目；当前最成熟的能力是公域流量日报与飞书联动，最需要谨慎使用的是依赖真实后台页面和写操作确认的自动化流程。
