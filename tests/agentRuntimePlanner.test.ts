...
describe('agent runtime planner proposal validation', () => {
  ...
  it('rejects all planner-hidden tools even when they exist in the internal registry', () => {
    ...
    expect(hiddenTools.map((tool) => tool.name)).toEqual(['operations.refreshActivityExecute', 'rental.mirrorWritebackState', 'rental.bulkPriceApply', 'rental.priceApply', 'rental.operationConfirmRequest', 'rental.perSpecPriceApply', 'rental.specDimApply']);
