...
const summary = { exposure: 0, publicVisits: 0, dashboardVisits: 0, createdOrders: 0, shippedOrders: 0, amount: 0, exposureVisitRate: 0, visitCreatedOrderRate: 0, visitShipmentRate: 0 };
...
const context = {
  ...
  weakClick: [],
  weakConversion: [{ identifier: '252', action: '提转化', reason: '访问多成交少' }],
  highPotential: [{ identifier: '253', action: '继续放量', reason: '高潜力' }],
...
} as PublicTrafficDataReportContext & { newProductPoolIds: string[] };

describe('buildAgentTaskPool', () => {
  it('combines report actions and new product pool into prioritized tasks', () => {
    expect(buildAgentTaskPool(context).map((item) => [item.productId, item.taskType, item.priority, item.status])).toEqual([
